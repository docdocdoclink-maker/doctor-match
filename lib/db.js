...
const dbPath = path.join(process.cwd(), "doctormatch.db");
const db = new Database(dbPath);
db.pragma("journal_mode = WAL");
// Next.js's production build loads this module from several parallel
// workers (to collect page data for each route). Without a busy timeout,
// concurrent writers can race and crash with SQLITE_BUSY/UNIQUE errors.
...
// Next.js's production build loads this module from several parallel
// workers, so two processes can both see a column missing and both try to
// add it. That's harmless — whichever loses the race just gets a
// "duplicate column" error here, which we ignore.
...
// Wrapped in an immediate transaction so the check-then-insert is atomic
// across the multiple processes that may load this module concurrently
// during a production build. Whichever process gets the write lock first
// seeds the data; any others wait, then see jobCount > 0 and skip.
const seedIfEmpty = db.transaction(() => {
  ...
  if (userCount === 0) {
    ...
    const info = db
      .prepare(
        "INSERT INTO users (email, password_hash, role, display_name) VALUES (?, ?, 'hospital', ?)"
      )
