...
if (!userCols.some((c) => c.name === "license_number")) {
  db.exec("ALTER TABLE users ADD COLUMN license_number TEXT;");
}
if (!userCols.some((c) => c.name === "has_seen_intro")) {
  db.exec("ALTER TABLE users ADD COLUMN has_seen_intro INTEGER NOT NULL DEFAULT 0;");
}
if (!userCols.some((c) => c.name === "agreed_to_terms_at")) {
...
if (convCols.length > 0 && !convCols.some((c) => c.name === "last_read_by_doctor")) {
  db.exec("ALTER TABLE conversations ADD COLUMN last_read_by_doctor TEXT;");
}
if (convCols.length > 0 && !convCols.some((c) => c.name === "last_read_by_hospital")) {
  db.exec("ALTER TABLE conversations ADD COLUMN last_read_by_hospital TEXT;");
}

// users.email_notify : per-user toggle for "email me on new chat messages".
if (!userCols.some((c) => c.name === "email_notify")) {
...
if (!userCols.some((c) => c.name === "verification_status")) {
  db.exec("ALTER TABLE users ADD COLUMN verification_status TEXT NOT NULL DEFAULT 'pending';");
