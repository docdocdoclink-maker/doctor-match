...
`);
  ...
  // jobs.pay_unit / jobs.pay_note : the posting form used to ask for pay_text
  // (free text, e.g. "日当 60,000円") AND pay_amount (a 万円 number just for
  // sorting) side by side, which read as two redundant "how much" fields.
  // pay_amount is now the single amount a hospital enters, paired with a
  // unit (日当/月給/時給/年俸) and an optional note ("経験に応じて相談可"
  // etc.); pay_text becomes a generated display string built from these
  // (see buildPayText in lib/pricing.js) rather than something typed
  // directly, so existing card/detail code reading job.pay_text is
  // unaffected.
  addColumnIfMissing(jobCols, "pay_unit", "ALTER TABLE jobs ADD COLUMN pay_unit TEXT;");
  addColumnIfMissing(jobCols, "pay_note", "ALTER TABLE jobs ADD COLUMN pay_note TEXT;");
  ...
  // jobs.hired_doctor_user_id / hired_reported_by / hired / hired_at :
  // superseded by conversations.hired / hired_at / hired_reported_by (see
  // below) once a job could have more than one hire — a job-level "the
  // hired doctor" singular didn't make sense anymore. Left in place
  // unused rather than dropped (SQLite column drops on a live table are
  // more trouble than they're worth for dead columns nobody reads).
  ...
  // jobs.headcount : optional "募集人数" — how many people this posting is
  // looking to fill. Purely informational (shown alongside how many are
  // already hired, via conversations.hired), since a job now stays open
  // until the hospital closes it manually regardless of hire count — this
  // doesn't drive any auto-close behavior.
  addColumnIfMissing(jobCols, "headcount", "ALTER TABLE jobs ADD COLUMN headcount INTEGER;");
...
`);
  ...
  // confirms it instead. See conversations.hired_reported_by below.
  ...
  // conversations.hired / hired_at / hired_reported_by : per-doctor hire
  // state, replacing the old job-level jobs.hired/hired_doctor_user_id/
  // hired_reported_by — a job can have several independent hires (e.g.
  // filling more than one shift from the same posting), each with its own
  // report/confirm/fee cycle, so "the" hire can't live on the job row
  // anymore. The job itself only closes when the hospital closes it
  // manually (see the close route) — hiring doesn't touch jobs.closed.
  addColumnIfMissing(convCols, "hired", "ALTER TABLE conversations ADD COLUMN hired INTEGER NOT NULL DEFAULT 0;");
  addColumnIfMissing(convCols, "hired_at", "ALTER TABLE conversations ADD COLUMN hired_at TEXT;");
  addColumnIfMissing(convCols, "hired_reported_by", "ALTER TABLE conversations ADD COLUMN hired_reported_by TEXT;");

  // conversations.fee_waived : whether THIS specific hire consumed the
  // hospital's one free first-hire (see users.first_hire_fee_waived_at
  // above). Recorded per-conversation, at the moment billing actually
  // triggers, so the UI can show what really happened for this hire even
  // after the hospital's free-hire allowance has since been used elsewhere —
  // recomputing from the hospital's current account state would wrongly
  // show later hires as free too.
  addColumnIfMissing(convCols, "fee_waived", "ALTER TABLE conversations ADD COLUMN fee_waived INTEGER NOT NULL DEFAULT 0;");
  ...
  // users.first_hire_fee_waived_at : replaces the old time-boxed launch
  // campaign (see lib/pricing.js) with a permanent "first contract is free"
  // policy, scoped per hospital account rather than per calendar date. NULL
  // until this hospital's first billable hire is reported/confirmed, at
  // which point it's stamped and every later hire is charged normally — see
  // isFirstHireFree() in lib/pricing.js and the hire/confirm-hire routes.
  addColumnIfMissing(userCols, "first_hire_fee_waived_at", "ALTER TABLE users ADD COLUMN first_hire_fee_waived_at TEXT;");

  // broadcast_sends : tracks the last time a given hospital's 一斉送信 reached
  // a given doctor, independent of which job triggered it. jobs.last_broadcast_at
  // only rate-limits repeat sends for the SAME job; a hospital with several open
  // postings could otherwise message the same doctor once per job per week. This
  // table lets the broadcast route also enforce "at most once per week per
  // hospital-doctor pair" across all of that hospital's jobs combined.
  db.exec(`
