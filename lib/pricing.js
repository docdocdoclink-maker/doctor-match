...
// Launch promotion: the success fee is waived through the end of the
// current fiscal year. Static end date — update or remove this (and the
// matching copy on the landing page / signup / terms) once the campaign
// is over rather than trying to make it configurable for a one-time thing.
const FREE_CAMPAIGN_END = "2027-03-31T23:59:59+09:00";

export function isFreeCampaignActive(now = new Date()) {
  return now <= new Date(FREE_CAMPAIGN_END);
}
