...
// Wraps an HTML body fragment in the branded shell (logo header + footer) so
// emails don't look like unstyled system output. Exported so call sites that
// need custom markup (a link, a blockquote, ...) can still get the same
// branding instead of writing their own bare <p> fallback.
export function renderEmailShell(bodyHtml) {
  return `
<!DOCTYPE html>
<html>
  <head>
...
// Plain text -> branded HTML, for call sites that don't build their own
// markup. Line breaks become <br>, blank lines become paragraph breaks.
